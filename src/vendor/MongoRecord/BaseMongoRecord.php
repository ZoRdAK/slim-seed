	...
	implements MongoRecord
{
	...
	protected static $attributesToSave = null;
	...
	public function save(array $options = array())
	{
        ...
        $attributes = $this->attributesToSave();
		$collection->save($attributes, $options);
        $this->attributes['_id'] = $attributes['_id'];
		$this->new = false;
		$this->afterSave();
    ...
    protected function attributesToSave()
    {
        if ( static::$attributesToSave == null ){
            return $this->attributes;
        }

        $attributesToSave = array();
        if ( isset($this->attributes['_id']) ){
            $attributesToSave['_id'] = $this->attributes['_id'];
        }
        foreach($this->attributes as $field=>$value){
            if ( in_array($field, static::$attributesToSave)){
                $attributesToSave[$field] = $value;
            }
        }
